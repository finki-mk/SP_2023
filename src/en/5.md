...
A half-Christmas tree is drawn with asterisks (*) and vertical bars (|) as follows:
...
So, in the first line, there is an asterisk. In the next line, there is a vertical bar. Further, through one line, we have either only a vertical bar or a vertical bar with several asterisks (one more than in the previous line). The half-Christmas tree ends with a line containing one vertical bar and N asterisks.

For a given N, print the half-Christmas tree according to the description.

For N=4, the output should be as shown in the example above.
